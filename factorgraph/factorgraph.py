...
class Graph(object):
    ...
    def factor(self, rvs, name="", potential=None, meta={}, debug=DEBUG_DEFAULT):
        ...
        for factor in self._factors:
            favtor_rv = factor.get_rvs()
            temp = [fac_rv.get_name() for fac_rv in favtor_rv]
            factor_rvs.append(temp)
        char_rvs = [str(rv) for rv in rvs]
        print(factor_rvs)
        print(char_rvs)
        print(type(char_rvs), type(factor_rvs))
        print(char_rvs in factor_rvs)
        if char_rvs in factor_rvs:
            print("Found")
            self._factors[factor_rvs.index(char_rvs)].update_potential(potential)
            return self._factors[factor_rvs.index(char_rvs)]
        else:
...
class Factor(object):
    ...
    def update_potential(self, potential):
        """
        Update the potential of the given factor
